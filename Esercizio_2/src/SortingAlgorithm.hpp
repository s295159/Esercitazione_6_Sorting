...
double durataBubble(vector<T>& data){
    ...
    double durationBubbleSort = chrono::duration_cast<chrono::milliseconds>(t_end - t_begin).count();
    return durationBubbleSort;
}

template<typename T>
double durataMerge(vector<T>& v){

    steady_clock::time_point t_begin = steady_clock::now();
    MergeSort(v);
    steady_clock::time_point t_end = steady_clock::now();
    double durationMergeSort = duration_cast<milliseconds>(t_end - t_begin).count();
    return durationMergeSort;
}
